...
eval(['load Calib_Results_left x_', num2str(ind), ' X_', num2str(ind), ' nx ny n_sq_x_1 n_sq_y_1 dX dY;']);
...
np = Rc_1_proj'*(np - Tc_1_proj*ones(1,size(np,2))); % X=-R^T*t+d*(R^T*u), we set d=1
...
P = -Rc_1_proj'*Tc_1_proj; % projector center
...
for i = 1:length(fc_cam)
   ...
   r = 1:ny_cam{i};
   [C,R] = meshgrid(c,r);
   Nc{i} = Rc_1_cam{1}*Rc_1_cam{i}'*pixel2ray([C(:) R(:)]'-1,fc_cam{i},cc_cam{i},kc_cam{i},alpha_c_cam{i}); % Nc{i} are relative to the 1st camera
   Oc{i} = Rc_1_cam{1}*Rc_1_cam{i}'*(-Tc_1_cam{i}) + Tc_1_cam{1}; % Oc{i} are relative to the 1st camera
%    Nc{i} = Rc_1_cam{i}'*pixel2ray([C(:) R(:)]'-1,fc_cam{i},cc_cam{i},kc_cam{i},alpha_c_cam{i}); % Nc{i} are relative to the 1st camera
%    Oc{i} = Rc_1_cam{i}'*(-Tc_1_cam{i}) % Oc{i} are relative to the 1st camera
end
