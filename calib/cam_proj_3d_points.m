...
for i=ind_active
    ...
    % the image name has to be 'base_name(_)', where (_) is index of the
    % image without 0 in the first digit.
%     I = double(rgb2gray(imread( [calib_name num2str(first_num+i-1) '.' format_image])));
    % the image name: 'base_name(_)(_)(_)(_)
    I = double(rgb2gray(imread(sprintf([proj_name, '%04d.', format_image], first_num+i-1))));
  
    display(['Processing Image number ' num2str(i)]);
    ProjectedGrid_2dpoints_cameraPlane = extract_grid(I,wintx,winty,fc,cc,kc,dX,dY);
    % X_i is the 3D positions in the camera coordinate system
    eval( ...
