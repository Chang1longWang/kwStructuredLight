...
objDir       = 'C:/Users/Daniela/Documents/3D_Recon/Data/synthetic_data/';
objName      = 'cube_sl';   % object name (should correspond to a data dir.)
...
nCam = 1; % there is only one camera
nBitPlanes = cell(1,nCam);
camDim = cell(1,nCam);
for camIdx = 1:nCam
   dataDir = [objDir, objName, '/'];
   % nBitPlanes not used, because the col and row frames might not be the
   % same
   nBitPlanes{camIdx} = ((length(dir(dataDir))-2)-2)/4; % ., .., .DS_Store
...
if strcmp(seqType,'Gray')
   [P,offset] = graycode(widthProj/dSampleProj,heightProj/dSampleProj);
else
   [P,offset] = bincode(widthProj/dSampleProj,heightProj/dSampleProj);
end

nColFrames = size(P{1, 1}, 3);
nRowFrames = size(P{2, 1}, 3);
...
for camIdx = 1:nCam
   ...
   if ~exist(dataDir,'dir')
      error([objDir, objName, ' is not available!']);
   end
   T{1}{camIdx} = imread([dataDir,num2str(41,'%0.04d'),'.jpg']);
   T{2}{camIdx} = imread([dataDir,num2str(40,'%0.04d'),'.jpg']);
   frameIdx = 0;
%    for j = 1:2 % column, row
%       for i = 1:nBitPlanes{camIdx} % number of projected patterns
...
%       end
%    end
    % read the column frames
    for i = 1 : nColFrames
         A{1,i}{camIdx} = imread([dataDir,num2str(frameIdx,'%0.04d'),'.jpg']);
         frameIdx = frameIdx + 1;
         B{1,i}{camIdx} = imread([dataDir,num2str(frameIdx,'%0.04d'),'.jpg']);
         frameIdx = frameIdx + 1;
    end
    % read the row frames
    for i = 1 : nRowFrames
         A{2,i}{camIdx} = imread([dataDir,num2str(frameIdx,'%0.04d'),'.jpg']);
         frameIdx = frameIdx + 1;
         B{2,i}{camIdx} = imread([dataDir,num2str(frameIdx,'%0.04d'),'.jpg']);
         frameIdx = frameIdx + 1;
    end
end
...
%          grayB = rgb2gray(im2double(B{j,i}{k}));
      ...
      if strcmp(seqType,'Gray')
         D{j,k} = gray2dec(G{j,k}(:,:,1:nFrames))-offset(j);
      else
         D{j,k} = bin2dec(G{j,k}(:,:,1:nFrames))-offset(j);
      end
      D{j,k}(~M{j,k}) = NaN;
   end
end
...
for i = 1:length(Nc)
   ...
   vrmlPoints([objDir, objName, '/', objName, '.wrl'],...
      vertices{i}(idx{i},[1 2 3]),colors{i}(idx{i},:));
%    writePly(['./data/',seqType,'/',objName,'/v',int2str(i),'.ply'], vertices{i}(idx{i},[1 3 2]),colors{i}(idx{i},:));
   mergedVertices = [mergedVertices; vertices{i}(idx{i},[1 2 3])];
   mergedColors = [mergedColors; colors{i}(idx{i},:)];
end
if length(Nc) > 1
   vrmlPoints([objDir ,objName,'/merged.wrl'],...
